...
# ------------------------------
ENV_FILE=".env.dev"
ACCESS_KEY=$(terraform output -raw eks_dev_user_access_key_id 2>/dev/null)
SECRET_KEY=$(terraform output -raw eks_dev_user_secret_access_key 2>/dev/null)
USER_NAME=$(terraform output -raw eks_dev_user_name 2>/dev/null)
ROLE_ARN=$(terraform output -raw eks_dev_role_arn 2>/dev/null)
SESSION_NAME="eks-dev-session"
CLUSTER_NAME="my-first-cluster"
NEW_PROFILE="assumed-dev-role"
...
if [ ! -f "$ENV_FILE" ]; then
  echo ".env.dev file not found! Creating from Terraform outputs..."
  ...
  if [ -z "$ACCESS_KEY" ]; then
    echo "Error: Failed to retrieve eks_dev_user_access_key_id. Check Terraform outputs."
    exit 1
  fi

  if [ -z "$SECRET_KEY" ]; then
    echo "Error: Failed to retrieve eks_dev_user_secret_access_key. Check Terraform outputs."
    exit 1
  fi
...
# ------------------------------
# 5. Export and Save Temporary Credentials
# ------------------------------
...
if [ -z "$AWS_ACCESS_KEY_ID" ] || [ -z "$AWS_SECRET_ACCESS_KEY" ] || [ -z "$AWS_SESSION_TOKEN" ]; then
  echo "Error: Failed to extract temporary credentials from assume-role-dev response."
  exit 1
fi
